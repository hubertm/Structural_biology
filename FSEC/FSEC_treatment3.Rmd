...
The idea is to get a number that reflects the amount of protein present in the correct oligomeric state. Following the methodology used in Gouaux papers, this number is either the height of or the area under the peak of the correct oligomeric form

The tutorial uses a set of 32 runs kindly provided by Hubert. The protein is a NTT-GFP fusion injected on a Superdex 200 column of 24 mL. The peak of interest appears to elute between 12 and 13 mL, which are thus taken as the boundaries for the integration.
...
cut -d ',' -f 7,8,15,16,23,24,31,32,39,40,47,48,55,56,63,64,71,72,79,80,87,88,95,96,103,104,111,112,119,120,127,128,135,136,143,144,151,152,159,160,167,168,175,176,183,184,191,192,199,200,207,208,215,216,223,224,231,232,239,240,247,248,255,256 Hubert1.csv > toto.csv

Here the *looong* list of columns to keep was generated with a *for* loop in R, and a quick manual post-treatment in TextWrangler to replace all spaces between number by commas.
...
sed -i -e 1,500d toto.csv 
sed -e :a -e '$d;N;2,5000ba' -e 'P;D' toto.csv > tmpfile ; mv tmpfile toto.csv

You can check that you actually removed 5500 lines (I get 9782)
...
The variable fluo is a kind of table. It contains `2n` sub-variables that are the volumes and fluorescence values for each run, with here *n=32*. It contains 9781 observations for each variable (yes one is missing, it reads the first line as the column names...). In R studio you can see the structure of the variable on the top right panel. In the console, you can access to one subvariable using its column number starting from 1, e.g. 5 is the volume for run 3, 6 is the fluo for run 3.
...
And have a look at one of the curves, here the first one with the volumes for sample 1

```{r}
plot(fluo[,1],fluo[,2],type="l",col="green")
```
